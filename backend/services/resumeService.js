...
try {
  ...
  if (geminiApiKey) {
    ...
    console.log('Resume service: Gemini API initialized successfully');
  } else {
    console.warn('GOOGLE_GEMINI_API_KEY or GEMINI_API_KEY not set for resume service. Using mock model.');
    model = {
      generateContent: async () => ({
        response: { text: () => JSON.stringify({ skills: [], atsScore: 75, analysis: "Mock analysis" }) }
      })
    };
  }
} catch (error) {
  console.error('Resume service: Error initializing Gemini:', error);
  model = {
    generateContent: async () => ({
      response: { text: () => JSON.stringify({ skills: [], atsScore: 75, analysis: "Mock analysis" }) }
    })
